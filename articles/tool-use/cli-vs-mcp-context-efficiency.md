...
## 七（续）、实证支撑：Terminal Agents Suffice

> 参见 `terminal-agents-enterprise-automation-2604-00073.md`

**arXiv:2604.00073**（ServiceNow / Mila / Université de Montréal，COLM 2026 under review）提供了迄今最系统的实证对比：

| 维度 | MCP Agent | Terminal Agent |
|------|-----------|----------------|
| ServiceNow 任务完成率 | 基线 | **相等或更优** |
| GitLab 任务完成率 | 基线 | **相等或更优** |
| ERPNext 任务完成率 | 基线 | **相等或更优** |
| Token 效率 | 低 | **高** |
| Multi-Agent 增益 | 有 | **有限** |

核心发现：**文档质量（而非工具抽象）是 Agent 能力的真正决定因素**——有高质量 API 文档的 Terminal Agent > 无文档的 MCP Agent。

这与本文 35x token 效率数据从两个维度（效率 + 任务完成率）指向同一结论：**在企业场景，MCP 的抽象成本往往不值得**。
...
- [Terminal Agents Suffice for Enterprise Automation](https://arxiv.org/abs/2604.00073) — Bechard et al., arXiv:2604.00073, 2026/03/31
